...
歯科予約システムのブロック枠を自動チェックし、Web管理画面で可視化するツール。

## 概要

さくら会グループの各分院が利用する予約システム（dent-sys.net / Stransa）にログインし、翌日の予約状況をスクレイピング。30分以上の連続ブロック枠を検出し、分院ごとのブロック状況をダッシュボードで確認できる。

## 対応システム

| システム | スロット間隔 | 30分ブロックの判定 |
|---------|-----------|----------------|
...
settings:
  consecutive_slots_required: 6   # 30分ブロックに必要な連続スロット数
  minimum_blocks_required: 4      # 不足判定の最小ブロック数
...
- **2025-02-19** 初期コミット: Flask Web管理画面付きのブロック枠チェッカー
