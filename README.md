...
さくら会グループの各分院が利用する予約システム（dent-sys.net / Stransa）にログインし、翌日の予約状況をスクレイピング。30分以上の連続空き枠（ブロック）を検出し、分院ごとの空き状況をダッシュボードで確認できる。

## 対応システム

| システム | スロット間隔 | 30分ブロック判定 |
|---------|-----------|---------------|
...
settings:
  consecutive_slots_required: 6   # 30分に必要な連続スロット数
